...
def controller(run_id='run54',target_channel=OPS_CHANNEL,outbox='reports/ops/outbox/ops_delivery_outbox.jsonl',state='reports/ops/outbox/ops_delivery_outbox_state.json',output_json='reports/editorial/run54-ops-delivery-controller.json',output_md='reports/editorial/run54-ops-delivery-controller.md'):
    if target_channel!=OPS_CHANNEL: raise ValueError('target_channel_must_be_AL-Hermoine-OPS')
    disc=discover(); write_discovery(disc,ROOT/'reports/editorial/run54-ops-autodiscovery.json',ROOT/'reports/editorial/run54-ops-autodiscovery.md')
    sj=ROOT/'reports/editorial/run54-ops-delivery-controller-status.json'; sm=ROOT/'reports/telegram/run54-ops-delivery-controller-status.md'
    payload={'status_metadata':meta('ops_delivery_controller',run_id,'ops_delivery_degraded_queued','warning','ops_alias_unresolved_retry_scheduled'),'target_channel':OPS_CHANNEL,'ops_alias_resolvable':disc['ops_alias_resolvable'],'fallback_channel_used':False,'summary':'Run 54 OPS delivery controller status. No fallback channel used.'}
    sj.parent.mkdir(parents=True,exist_ok=True); sj.write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n'); sm.parent.mkdir(parents=True,exist_ok=True); sm.write_text('# Run 54 OPS delivery controller status\n\n```json\n'+json.dumps(payload,indent=2,sort_keys=True)+'\n```\n')
    ...
    Path(ROOT/output_md).parent.mkdir(parents=True,exist_ok=True); Path(ROOT/output_md).write_text('# Run 54 OPS delivery controller\n\n```json\n'+json.dumps(report,indent=2,sort_keys=True)+'\n```\n')
    return report
def main(argv=None):
    ap=argparse.ArgumentParser(); ap.add_argument('--run-id',default='run54'); ap.add_argument('--target-channel',default=OPS_CHANNEL); ap.add_argument('--outbox',default='reports/ops/outbox/ops_delivery_outbox.jsonl'); ap.add_argument('--state',default='reports/ops/outbox/ops_delivery_outbox_state.json'); ap.add_argument('--output-json',default='reports/editorial/run54-ops-delivery-controller.json'); ap.add_argument('--output-md',default='reports/editorial/run54-ops-delivery-controller.md')
