...
Consumes Run 27 rebuilt-input preflight approval and Run 26 enriched author-input
metadata, then uses GPT-5.5 through the closed_loop_editorial profile via the
shared strict-JSON Hermes bridge to create one short draft canary v2 in report
artifacts only.

This is not authoring approval, not publication approval, not claim insertion,
not chapter-ready prose, and never writes to docs/book.
"""
...
MODE = "llm_author_draft_canary_v2"
DEFAULT_PREFLIGHT_REPORT = f"reports/editorial/{RUN_ID_DEFAULT}-rebuilt-author-input-preflight-run27.json"
DEFAULT_REBUILD_REPORT = f"reports/editorial/{RUN_ID_DEFAULT}-author-draft-input-rebuild-run26.json"
...
ALLOWED_CANARY_TYPES = {"caveat_only_author_draft_canary_v2", "safe_reports_only_draft_canary"}
ALLOWED_CANARY_DECISIONS = {
    "draft_canary_v2_created",
    "safe_reports_only",
    "needs_more_sources",
...
REQUIRED_CANARY_FIELDS = [
    ...
    "draft_canary_text",
    "word_count",
    "usefulness_improvement_over_run24",
    "required_caveats",
    "do_not_say",
...
BAD_TEXT_PATTERNS = [
    ...
    (re.compile(r"\bbeyond\s+migration/setup/import\s+tooling\b|\bbeyond\s+migration\s+setup\s+import\s+tooling\b", re.I), "beyond migration/setup/import tooling overgeneralization"),
]
SAFE_TARGET_CHAPTER_STATUSES = {"suggested_only", "not_assigned"}
...
def load_json(path: str | Path, label: str) -> dict[str, Any]:
    p = resolve(path)
    if not p.exists():
        raise CanaryV2Error(f"missing input {label}: {p}")
    try:
        data = json.loads(p.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        raise CanaryV2Error(f"invalid JSON in {label}: {exc}") from exc
    if not isinstance(data, dict):
        raise CanaryV2Error(f"{label} must be JSON object")
    return data
...
def connect_readonly(path: Path) -> sqlite3.Connection:
    if not path.exists():
        raise CanaryV2Error(f"missing SQLite DB: {path}")
...
def has_chapter_ready_field(obj: Any) -> bool:
    if isinstance(obj, dict):
        ...
        return any(has_chapter_ready_field(item) for item in obj)
    return False


def require_false_flags(obj: dict[str, Any], label: str) -> None:
    if obj.get("advisory_only") is not True:
        raise CanaryV2Error(f"{label} safety flag invalid: advisory_only")
    for flag in FALSE_FLAGS:
        if obj.get(flag) is not False:
            raise CanaryV2Error(f"{label} safety flag invalid: {flag}")
...
def validate_caveat_and_dns(obj: dict[str, Any], label: str) -> None:
    ...
    if not isinstance(caveats, list) or REQUIRED_CAVEAT not in joined(caveats):
        raise CanaryV2Error(f"{label} missing required caveat")
    dns = obj.get("do_not_say")
    if not isinstance(dns, list) or not dns:
        raise CanaryV2Error(f"{label} missing do_not_say guidance")
    dns_text = joined(dns)
    for required in REQUIRED_DO_NOT_SAY_SUBSTRINGS:
        if required not in dns_text:
            raise CanaryV2Error(f"{label} do_not_say missing required guidance: {required}")


def validate_reports(preflight: dict[str, Any], rebuild: dict[str, Any]) -> None:
    if preflight.get("mode") != "llm_rebuilt_author_input_preflight":
        raise CanaryV2Error("Run 27 preflight report mode mismatch")
    if preflight.get("report_only") is not True or preflight.get("llm_used") is not True:
        raise CanaryV2Error("Run 27 preflight report must be report_only with GPT-5.5 used")
    if preflight.get("provider") != "copilot" or preflight.get("model") != "gpt-5.5" or preflight.get("bridge") != "hermes_cli" or preflight.get("model_profile") != "closed_loop_editorial":
        raise CanaryV2Error("Run 27 preflight report has unexpected model/profile metadata")
    if not isinstance(preflight.get("rebuilt_input_preflight_reviews"), list):
        raise CanaryV2Error("Run 27 preflight report missing rebuilt_input_preflight_reviews list")
    if rebuild.get("mode") != "llm_rebuild_author_draft_input":
        raise CanaryV2Error("Run 26 rebuild report mode mismatch")
    if rebuild.get("report_only") is not True:
        raise CanaryV2Error("Run 26 rebuild report must be report_only")
    if not isinstance(rebuild.get("rebuilt_author_draft_inputs"), list):
        raise CanaryV2Error("Run 26 rebuild report missing rebuilt_author_draft_inputs list")


def validate_preflight_review(review: dict[str, Any]) -> None:
    if not isinstance(review, dict):
        raise CanaryV2Error("preflight review must be object")
    rid = review.get("rebuilt_input_id", "unknown")
    require_false_flags(review, f"Run 27 preflight review {rid}")
    if review.get("preflight_decision") != "rebuilt_input_canary_ready":
        raise CanaryV2Error(f"Run 27 preflight review {rid} not canary-ready")
    if review.get("canary_readiness") != "ready_for_second_controlled_caveat_only_author_draft_canary":
        raise CanaryV2Error(f"Run 27 preflight review {rid} not ready for second canary")
    if review.get("closed_loop_disposition") != "caveat_only":
        raise CanaryV2Error(f"Run 27 preflight review {rid} disposition not caveat_only")
    if review.get("recommended_next_stage") != "run_second_controlled_caveat_only_author_draft_canary":
        raise CanaryV2Error(f"Run 27 preflight review {rid} next stage mismatch")
    validate_caveat_and_dns(review, f"Run 27 preflight review {rid}")


def validate_rebuilt_input(pkg: dict[str, Any]) -> None:
    if not isinstance(pkg, dict):
        raise CanaryV2Error("rebuilt input must be object")
    rid = pkg.get("rebuilt_input_id", "unknown")
    require_false_flags(pkg, f"Run 26 rebuilt input {rid}")
    if has_chapter_ready_field(pkg):
        raise CanaryV2Error(f"Run 26 rebuilt input {rid} contains chapter-ready prose field")
    if pkg.get("raw_capture_dependency") is True or pkg.get("raw_content_stored") is True:
        raise CanaryV2Error(f"Run 26 rebuilt input {rid} has forbidden raw capture dependency")
    if pkg.get("rebuilt_input_type") != "enriched_caveat_only_author_draft_input":
        raise CanaryV2Error(f"Run 26 rebuilt input {rid} type mismatch")
    if pkg.get("rebuilt_input_decision") != "rebuilt_draft_input_candidate":
        raise CanaryV2Error(f"Run 26 rebuilt input {rid} decision mismatch")
    if pkg.get("rebuilt_input_use") != "caveat_only":
        raise CanaryV2Error(f"Run 26 rebuilt input {rid} use mismatch")
    if pkg.get("target_chapter_status") not in SAFE_TARGET_CHAPTER_STATUSES:
        raise CanaryV2Error(f"Run 26 rebuilt input {rid} target_chapter_status mismatch")
    validate_caveat_and_dns(pkg, f"Run 26 rebuilt input {rid}")
    for field in [
        ...
        value = pkg.get(field)
        if field in {"later_canary_instruction_seed", "why_prior_canary_was_not_useful"}:
            if not isinstance(value, str) or not value.strip():
                raise CanaryV2Error(f"Run 26 rebuilt input {rid} missing {field}")
        elif not isinstance(value, list) or not value:
            raise CanaryV2Error(f"Run 26 rebuilt input {rid} missing {field}")


def select_rebuilt_inputs(preflight: dict[str, Any], rebuild: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    rebuilt_by_id = {str(p.get("rebuilt_input_id")): p for p in rebuild.get("rebuilt_author_draft_inputs", []) if isinstance(p, dict)}
    selected: list[dict[str, Any]] = []
    excluded: list[dict[str, Any]] = []
    for review in preflight.get("rebuilt_input_preflight_reviews", []):
        try:
            validate_preflight_review(review)
        except CanaryV2Error as exc:
            if str(exc).lower().find("not canary-ready") >= 0:
                cp = dict(review) if isinstance(review, dict) else {"raw_review": review}
                cp["excluded_reason"] = "does_not_match_run28_selection_rule"
                excluded.append(cp)
                continue
            raise
        rid = str(review.get("rebuilt_input_id"))
        pkg = rebuilt_by_id.get(rid)
        if not pkg:
            raise CanaryV2Error(f"Run 27 preflight references missing Run 26 rebuilt input: {rid}")
        validate_rebuilt_input(pkg)
        merged = dict(pkg)
        merged["run27_preflight_review"] = review
...
def atom_used_in_text(text: str, atoms: list[Any]) -> bool:
    nt = normalized(text)
    phrase_markers = [
        "documentation summaries name hermes in migration tooling context",
        "documentation summaries name hermes in setup tooling context",
        "documentation summaries name hermes in migration and setup tooling contexts",
        "documentation summaries name hermes in migration setup tooling contexts",
        "source cluster is singleton based",
        "singleton cluster",
        "persisted rereview note is downstream eligible but narrow",
        "candidate source identifiers",
        "source review identifier",
        "manifest item identifier",
        "safe narrative role is ecosystem tooling adjacency",
    ]
    for marker in phrase_markers:
        if marker in nt:
            return True
    for atom in atoms:
        atom_text = str(atom or "")
        if not atom_text:
            continue
        na = normalized(atom_text)
        if len(na) > 20 and na in nt:
            return True
        # Allow exact atom text without a leading OpenClaw subject.
        if na.startswith("openclaw ") and na[9:] in nt:
            return True
    return False


def validate_canary_text(text: str, source: dict[str, Any]) -> int:
    if "\n" in text.strip():
    ...
    wc = word_count(text)
    if wc > 110:
        raise ValueError("draft_canary_text exceeds 110 words")
    if REQUIRED_CAVEAT not in text:
        raise ValueError("draft_canary_text must preserve required caveat exactly or more conservatively")
    if normalized(text) == normalized(REQUIRED_CAVEAT):
        raise ValueError("draft_canary_text merely restates required caveat")
    lower = text.lower()
    if any(secret in lower for secret in ["api key", "oauth", "cookie", "bearer token", "password", "secret key"]):
    ...
    if not atom_used_in_text(text, source.get("evidence_bound_factual_atoms") or []):
        raise ValueError("draft_canary_text does not use an evidence-bound factual atom")
    return wc
...
def validate_canary(canary: dict[str, Any], selected_by_id: dict[str, dict[str, Any]]) -> None:
    ...
    rid = require_str(canary.get("rebuilt_input_id"), "rebuilt_input_id")
    if rid not in selected_by_id:
        raise ValueError(f"draft canary references unknown selected rebuilt input: {rid}")
    ...
    for flag in FALSE_FLAGS:
        if canary.get(flag) is not False:
            raise ValueError(f"draft canary {flag} must be false")
    if canary.get("singleton_canary") is not True:
        raise ValueError("singleton_canary must be true for Run 28 canary")
    ...
    for field in ["draft_canary_id", "prior_draft_input_id", "prior_draft_canary_id", "source_packet_id", "source_cluster_id", "draft_canary_text", "usefulness_improvement_over_run24", "caveat_compliance_notes", "evidence_usage_notes", "residual_risk", "target_chapter_status", "evidence_narrowness_warning"]:
        require_str(canary.get(field), field)
    if canary.get("target_chapter_status") not in SAFE_TARGET_CHAPTER_STATUSES:
        raise ValueError("target_chapter_status must remain suggested_only or not_assigned")
    if REQUIRED_CAVEAT not in joined(canary.get("required_caveats")):
        raise ValueError("draft canary missing required caveat")
    dns = joined(canary.get("do_not_say"))
    for required in REQUIRED_DO_NOT_SAY_SUBSTRINGS:
        if required not in dns:
            raise ValueError(f"draft canary do_not_say missing required guidance: {required}")
    source = selected_by_id[rid]
    computed = validate_canary_text(canary["draft_canary_text"], source)
...
def make_validator(selected: list[dict[str, Any]]):
    selected_by_id = {str(p["rebuilt_input_id"]): p for p in selected}
    ...
    def validator(obj: dict[str, Any]) -> None:
        ...
        for canary in canaries:
            ...
            rid = canary["rebuilt_input_id"]
            if rid in seen:
                raise ValueError(f"duplicate rebuilt_input_id canary: {rid}")
            seen.add(rid)
...
def build_prompt(selected: list[dict[str, Any]], excluded: list[dict[str, Any]], run_id: str) -> str:
        ...
        "draft_canaries": [
            {
                ...
                "rebuilt_input_id": "string from selected_rebuilt_inputs",
                "prior_draft_input_id": "string from selected_rebuilt_inputs",
                "prior_draft_canary_id": "string from selected_rebuilt_inputs",
                "source_packet_id": "string from selected_rebuilt_inputs",
                "source_cluster_id": "string from selected_rebuilt_inputs",
                "source_note_ids": ["strings from selected_rebuilt_inputs"],
                "source_review_ids": ["strings from selected_rebuilt_inputs"],
                "source_ids": ["strings from selected_rebuilt_inputs"],
                "manifest_item_ids": ["strings from selected_rebuilt_inputs"],
                "candidate_source_ids": ["strings from selected_rebuilt_inputs"],
                "draft_canary_type": "choose exactly one string: caveat_only_author_draft_canary_v2 | safe_reports_only_draft_canary",
                "draft_canary_decision": "choose exactly one string: draft_canary_v2_created | safe_reports_only | needs_more_sources | source_context_unclear | exclude_from_authoring | contradiction_review_required",
                "draft_canary_use": "choose exactly one string: caveat_only | safe_reports_only | not_for_chapter_update | not_for_publication",
                "draft_canary_text": "one paragraph string, <=110 words, no headings/bullets, preserve required caveat exactly or more conservatively, use at least one evidence-bound factual atom",
                "word_count": "integer exact count of words in draft_canary_text",
                "usefulness_improvement_over_run24": "single non-empty string explaining why it is more useful than Run 24 or why not",
                "required_caveats": [REQUIRED_CAVEAT],
                "do_not_say": REQUIRED_DO_NOT_SAY,
                "caveat_compliance_notes": "single non-empty string, not a list",
                "evidence_usage_notes": "single non-empty string identifying the evidence-bound atom(s) used",
                "evidence_limitations": ["strings"],
                "residual_risk": "single non-empty string, not a list",
                "provenance_paths": ["strings copied from selected_rebuilt_inputs plus Run 26/27 report paths"],
                "target_chapter_status": "suggested_only or not_assigned",
    ...
    payload = {
        "run_id": run_id,
        "task": "Run 28 second controlled report-only caveat-only author-draft canary v2",
        ...
        "instructions": [
            ...
            "Create exactly one short controlled draft_canary_text per selected input, only inside the JSON report object.",
            "draft_canary_text must be one paragraph, no headings, no bullets, and at most 110 words.",
            "draft_canary_text must not merely restate the Run 24 required caveat; it must use at least one evidence-bound factual atom from selected_rebuilt_inputs.",
            "Preserve the required caveat exactly or use a more conservative equivalent; exact preservation is preferred.",
            "Do not say Hermes is a runtime dependency of OpenClaw.",
            "Do not say Hermes is the general operating environment for OpenClaw.",
            "Do not say OpenClaw requires Hermes for web or phone access.",
            "Do not generalize beyond migration/setup/import tooling contexts unless additional sources support it.",
            ...
            "Even when draft_canary_decision=draft_canary_v2_created, set advisory_only=true, draft_canary_only=true, author_allowed=false, publication_approved=false, eligible_for_claim_insertion=false, eligible_for_authoring=false, eligible_for_publication=false, and chapter_update_allowed=false.",
...
def safety_flags() -> dict[str, bool]:
    return {
        ...
        "draft_canary_is_final_prose": False,
...
def build_payload(args: argparse.Namespace) -> dict[str, Any]:
    ...
    preflight = load_json(preflight_path, "Run 27 rebuilt input preflight report")
    rebuild = load_json(rebuild_path, "Run 26 rebuilt author input report")
    validate_reports(preflight, rebuild)
    try:
        profile = load_model_profile(args.reasoning_profile)
    except ModelProfileError as exc:
        raise CanaryV2Error(f"model_profile_error:{exc}") from exc
    selected, excluded = select_rebuilt_inputs(preflight, rebuild)
    if not selected:
        raise CanaryV2Error("no selected canary-ready rebuilt inputs")
    db = db_path()
    with connect_readonly(db) as con:
        ...
        try:
            llm = call_high_reasoning_json(
                prompt,
                schema_name="run28_author_draft_canary_v2",
            ...
            raise CanaryV2Error(str(exc.result.get("error") or exc)) from exc
    ...
    payload = {
        ...
        "mode": MODE,
        "input_paths": {"preflight_report": rel(preflight_path), "rebuild_report": rel(rebuild_path), "sqlite_db": rel(db)},
        "report_only": True,
    ...
    if not payload["llm_used"] or payload["reasoning_status"] != "high_reasoning_used":
        raise CanaryV2Error("high reasoning was not used")
    if payload["provider"] != "copilot" or payload["model"] != "gpt-5.5" or payload["bridge"] != "hermes_cli" or payload["model_profile"] != args.reasoning_profile:
        raise CanaryV2Error("unexpected high-reasoning provider/model/bridge/profile")
    if payload["changed_source_notes"] or payload["claims_inserted"] or payload["editorial_reviews_inserted"] or payload["source_status_changed"] or payload["claim_status_changed"] or payload["editorial_status_changed"]:
        raise CanaryV2Error("forbidden DB/status delta detected during report-only canary v2")
    return payload


def render_markdown(payload: dict[str, Any]) -> str:
    lines = [
        f"# Run 28 author-draft canary v2 — {payload['run_id']}",
        "",
        "## Summary",
        "",
        ...
        f"- Draft canaries v2: `{payload['draft_canary_count']}`",
        f"- Excluded rebuilt inputs: `{payload['excluded_rebuilt_input_count']}`",
        "",
        "## Selected rebuilt input",
        "",
    ]
    for item in payload["selected_rebuilt_inputs"]:
        lines += [
            f"- rebuilt_input_id: `{item['rebuilt_input_id']}`",
            f"- rebuilt_input_type: `{item['rebuilt_input_type']}`",
            f"- rebuilt_input_use: `{item['rebuilt_input_use']}`",
            f"- target_chapter_status: `{item['target_chapter_status']}`",
            "",
        ]
    lines += ["## Draft canary v2 created", ""]
    for c in payload["draft_canaries"]:
        lines += [
            f"- draft_canary_id: `{c['draft_canary_id']}`",
            f"- rebuilt_input_id: `{c['rebuilt_input_id']}`",
            ...
            "",
            c["usefulness_improvement_over_run24"],
            "",
...
def write_reports(payload: dict[str, Any], output_dir: Path, suffix: str) -> dict[str, str]:
    ...
    stem = f"{payload['run_id']}-author-draft-canary-v2-{suffix}" if suffix else f"{payload['run_id']}-author-draft-canary-v2"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    ap.add_argument("--rebuild-report", default=DEFAULT_REBUILD_REPORT)
    ap.add_argument("--output-dir", default="reports/editorial")
    ap.add_argument("--report-suffix", default="run28")
