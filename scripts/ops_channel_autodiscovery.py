...
def discover():
    av=aliases(); inspected=[inspect_path(p) for p in INSPECT]
    ops_found=any(OPS_CHANNEL in json.dumps(r) for r in inspected) or any(OPS_CHANNEL in a for a in av); resolvable=any(OPS_CHANNEL in a for a in av)
    cand=[]
    for r in inspected:
    ...
    return {'status_metadata':ts(status),'active_profile':'default','active_channel_directory':'/root/.hermes/channel_directory.json','available_telegram_aliases':av,'ops_alias_found':ops_found,'ops_alias_resolvable':resolvable,'candidate_aliases':cand,'candidate_confidence':'high' if resolvable else 'none' if not cand else 'low','can_repair_safely':bool(resolvable),'repair_action':'none_needed' if resolvable else 'no_safe_repair_candidate_do_not_invent_alias','reload_required':not resolvable,'secrets_redacted':True,'fallback_channel_used':False,'files_inspected':inspected}
